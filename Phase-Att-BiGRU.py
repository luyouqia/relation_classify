...
def build_model(batch_size, sent_size, e1e2_phrase_size, vec_size, embedding_weights, nb_class):
    ...
    word_ori = Lambda(get_sent, output_shape=(sent_size, vec_size))(input)
    ...
    #sentence_mid = Lambda(get_sent, output_shape=(sent_size, vec_size+2*pf_dim))(sentence_ori)
    #entity1 = Lambda(get_e1, output_shape=(vec_size+2*pf_dim, ))(sentence_ori)
    #entity2 = Lambda(get_e2, output_shape=(vec_size+2*pf_dim, ))(sentence_ori)
    sentence = merge([sentence_ori, vector_weight], mode='mul')
    ...
    e1_e2_1 = Convolution1D(100, 3, input_shape=(e1e2_phrase_size, vec_size),
                            activation='relu', border_mode='same')(input_e1e2)
    e1_e2_1 = MaxPooling1D(e1e2_phrase_size - 2)(e1_e2_1)
    e1_e2_1 = Flatten()(e1_e2_1)

    e1_e2_2 = Convolution1D(100, 4, input_shape=(e1e2_phrase_size, vec_size),
                            activation='relu', border_mode='same')(input_e1e2)
    e1_e2_2 = MaxPooling1D(e1e2_phrase_size - 3)(e1_e2_2)
    e1_e2_2 = Flatten()(e1_e2_2)

    e1_e2_3 = Convolution1D(100, 5, input_shape=(e1e2_phrase_size, vec_size),
                            activation='relu', border_mode='same')(input_e1e2)
    ...
    e1_e2_dense = Dense(200, activation='tanh')(e1e2_merge)
    ...
    out = Dense(100, activation='tanh')(hidden_attention_reshape)
    #out = Activation('tanh')(hidden_attention_reshape)
...
def train_model(x1_train, y_train, pf1_train, pf2_train, train_e1e2_phrase, vector_train, embedding_weights, save_to, split, nb_class):
    pair_wise_cnn = build_model(50, 103, 47, 300, embedding_weights, nb_class)
    ...
    v = pair_wise_cnn.fit([x1_train, pf1_train, pf2_train, train_e1e2_phrase, vector_train], [y_train],
                          nb_epoch=30, batch_size=20,
                          shuffle=True,
...
def test_model(x1_test, y_test, pf1_test, pf2_test, test_e1e2_phrase, vector_test, json_string, weight_path):
...
if __name__ == '__main__':
    ...
    json_string = train_model(x_train, y_train, pf1_train, pf2_train, train_e1e2_phrase,
                              vector_train, embedding_weights, save_to, split, nb_class)

    # json_string = open('model_data/ly_model_architecture.json', 'r').read()

    # weight_path = './model_data/lymouth.hdf5'
    # score, prediction = test_model(
    # x_test, y_test, pf1_test, pf2_test, test_e1e2_phrase, vector_test,
    # json_string, weight_path)
    ...
    #score,prediction = test_model(x_train,y_train,pf1_train,pf2_train,vector_train,json_string)
